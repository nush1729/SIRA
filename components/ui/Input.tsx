...
export interface InputProps extends InputHTMLAttributes<HTMLInputElement> { label: string; hint?: string; error?: string; hideLabel?: boolean }
export function Input({ label, hint, error, hideLabel = false, id, className = "", ...props }: InputProps) {
  const generatedId = useId();
  const inputId = id ?? generatedId;
  return <div className="space-y-1.5">
    <label htmlFor={inputId} className={hideLabel ? "sr-only" : "block text-sm font-medium text-zinc-800"}>{label}</label>
